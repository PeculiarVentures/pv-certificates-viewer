import { id_ActivityDescription } from '@peculiar/asn1-ntqwac';
import { makeAttrRaw } from '../../../tests/test_utils';
import { ActivityDescriptionParser } from './activity_description';

const ACTIVITY_HEX = '';

describe('ActivityDescriptionParser', () => {
  const parser = new ActivityDescriptionParser();

  it('exposes the correct OID', () => {
    expect(parser.oids).toEqual([id_ActivityDescription]);
    expect(parser.oids).toContain('0.4.0.9496.6');
  });

  it('parses activity description fields', () => {
    const result = parser.parse(makeAttrRaw(id_ActivityDescription, ACTIVITY_HEX));

    expect(result.oid).toBe('0.4.0.9496.6');
    expect(result.children).toHaveLength(4);
    expect(result.children[0]?.title).toBe('Code Authority');
    expect(result.children[0]?.children?.[0]).toEqual({
      title: 'RFC 822 Name', value: 'NACEBEL',
    });
    expect(result.children[1]?.title).toBe('Code ID');
    expect(result.children[1]?.children?.[0]).toEqual({
      title: 'RFC 822 Name', value: '66.010',
    });
    expect(result.children[2]).toEqual({
      title: 'Short Name', value: 'Computer programming activities',
    });
    expect(result.children[3]?.title).toBe('Short Description');
    expect(result.children[3]?.value).toContain('La société a pour objet');
  });
});
