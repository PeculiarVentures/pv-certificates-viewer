...
describe('parseAttribute', () => {
  it('dispatches to the matching parser for a known OID', () => {
    ...
    expect(
      parseAttribute(makeAttrRaw('1.2.840.113549.1.9.7', '0c06736563726574'), makeRegistry()),
    ).toEqual({
  ...
  it('falls back to unknown when OID has no registered parser', () => {
    expect(parseAttribute(makeAttrRaw('1.2.3.4.5', '01020304'), makeRegistry())).toEqual({
      oid: '1.2.3.4.5',
      children: [
        {
          title: 'Raw Value',
  ...
  it('falls back to unknown when the parser throws', () => {
    ...
    expect(
      parseAttribute(makeAttrRaw('1.2.840.113549.1.9.7', '0101010101'), makeRegistry()),
    ).toEqual({
      oid: '1.2.840.113549.1.9.7',
      children: [
        {
