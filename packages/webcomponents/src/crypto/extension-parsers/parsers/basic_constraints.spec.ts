...
describe('BasicConstraintsParser', () => {
  ...
  it('parses cA=true with pathLenConstraint=0', () => {
    expect(parser.parse(makeExtRaw(id_ce_basicConstraints, '30060101ff020100', true))).toEqual({
      oid: '2.5.29.19',
      critical: true,
      children: [
        {
  ...
  it('parses cA=true without pathLenConstraint', () => {
    expect(parser.parse(makeExtRaw(id_ce_basicConstraints, '30030101ff', true))).toEqual({
      oid: '2.5.29.19',
      critical: true,
      children: [
        {
  ...
  it('defaults cA to false when absent', () => {
    expect(parser.parse(makeExtRaw(id_ce_basicConstraints, '3000'))).toEqual({
      oid: '2.5.29.19',
      critical: false,
      children: [
        {
