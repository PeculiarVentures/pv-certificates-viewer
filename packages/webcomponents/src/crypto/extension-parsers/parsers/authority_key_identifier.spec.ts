...
describe('AuthorityKeyIdentifierParser', () => {
  ...
  it('parses keyIdentifier only', () => {
    expect(
      parser.parse(
        makeExtRaw(id_ce_authorityKeyIdentifier, '301280100102030405060708090a0b0c0d0e0f10'),
      ),
    ).toEqual({
      oid: '2.5.29.35',
      critical: false,
  ...
  it('parses keyIdentifier and authorityCertSerialNumber', () => {
    expect(
      parser.parse(makeExtRaw(id_ce_authorityKeyIdentifier, '300a80040a0b0c0d82020102')),
    ).toEqual({
      oid: '2.5.29.35',
      critical: false,
      children: [
  ...
  it('returns an empty children array when no fields are present', () => {
    expect(parser.parse(makeExtRaw(id_ce_authorityKeyIdentifier, '3000'))).toEqual({
      oid: '2.5.29.35',
      critical: false,
      children: [],
    });
