...
describe('PolicyConstraintsParser', () => {
  ...
  it('parses both requireExplicitPolicy and inhibitPolicyMapping', () => {
    ...
    expect(
      parser.parse(
        makeExtRaw(id_ce_policyConstraints, '30068001028101 00'.replace(/\s/g, ''), true),
      ),
    ).toEqual({
      oid: '2.5.29.36',
      critical: true,
      children: [
        {
          title: 'Require Explicit Policy',
          value: 2,
        },
        {
          title: 'Inhibit Policy Mapping',
          value: 0,
        },
      ],
    });
  ...
  it('decodes multi-byte integer values correctly', () => {
    ...
    expect(
      parser.parse(makeExtRaw(id_ce_policyConstraints, '3004800201 2c'.replace(/\s/g, ''), true)),
    ).toEqual({
      oid: '2.5.29.36',
      critical: true,
      children: [
