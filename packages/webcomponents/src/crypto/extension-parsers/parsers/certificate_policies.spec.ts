...
describe('CertificatePoliciesParser', () => {
  ...
  it('parses a single policy with no qualifiers', () => {
    expect(parser.parse(makeExtRaw(id_ce_certificatePolicies, '300830060604551d2000'))).toEqual({
      oid: '2.5.29.32',
      critical: false,
      children: [
        {
  ...
  it('parses multiple policies', () => {
    expect(
      parser.parse(makeExtRaw(id_ce_certificatePolicies, '300f30060604551d2000300506032a0304')),
    ).toEqual({
      oid: '2.5.29.32',
      critical: false,
      children: [
