...
describe('parseExtension', () => {
  it('dispatches to the matching parser for a known OID', () => {
    ...
    expect(
      parseExtension(makeExtRaw(id_ce_basicConstraints, '30060101ff020100', true), makeRegistry()),
    ).toEqual({
      oid: '2.5.29.19',
      critical: true,
      children: [
  ...
  it('falls back to unknown when OID has no registered parser', () => {
    expect(parseExtension(makeExtRaw('1.2.3.4.5', '01020304'), makeRegistry())).toEqual({
      oid: '1.2.3.4.5',
      critical: false,
      children: [
        {
  ...
  it('falls back to unknown when the parser throws', () => {
    expect(
      parseExtension(makeExtRaw(id_ce_basicConstraints, '0101010101'), makeRegistry()),
    ).toEqual({
      oid: '2.5.29.19',
      critical: false,
      children: [
