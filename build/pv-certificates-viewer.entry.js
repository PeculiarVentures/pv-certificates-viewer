import { r as registerInstance, h } from './core-81618ee4.js';
...
const CertificatesViewer = class {
    ...
    async certificatesDecodeAndSet() {
        ...
        for (let value of this.certificatesPropParsed) {
            const certificate = new Certificate$1(value);
    ...
    renderDN(item) {
        return Object.keys(item).map(subject => {
            return (h("p", null, h("span", null, item[subject].name), h("span", null, item[subject].value)));
        });
    }
    renderMetaData(item) {
        return ([
            h("p", null, h("span", null, "Serial number:"), h("span", null, item.serialNumber)),
            h("p", null, h("span", null, "Version:"), h("span", null, item.version)),
            h("p", null, h("span", null, "Validity:"), h("span", null, item.validity, " days")),
            h("p", null, h("span", null, "Issued:"), h("span", null, dayjs_min(item.notBefore).format('ddd, MMM D, YYYY h:mm:ss'))),
            h("p", null, h("span", null, "Expired:"), h("span", null, dayjs_min(item.notAfter).format('ddd, MMM D, YYYY h:mm:ss'))),
        ]);
    }
    renderCertificates() {
        return this.certificatesDecoded.map(certificate => {
            ...
            return ([
                h("tr", { class: isExpanded && 'fill_grey' }, h("td", null, certificate.commonName), h("td", null, certificate.fingerprint), h("td", null, h("button", { onClick: this.setExpanded.bind(this, certificate.serialNumber) }, "Details"), h("button", { onClick: certificate.downloadAsPEM }, "Download PEM"), h("button", { onClick: certificate.downloadAsDER }, "Download DER")), h("td", null)),
                isExpanded && (h("tr", { class: "fill_grey" }, h("td", { colSpan: 1 }, h("span", null, "Subject DN:"), this.renderDN(certificate.subject)), h("td", { colSpan: 1 }, h("span", null, "Issuer DN:"), this.renderDN(certificate.issuer)), h("td", { colSpan: 2 }, this.renderMetaData(certificate)))),
            ]);
        });
    }
    render() {
        return (h("table", null, h("thead", null, h("tr", null, h("th", null, "Name"), h("th", null, "Fingerprint (SHA-1)"), h("th", null, "Actions"), h("th", null, "Test URLs"))), h("tbody", null, this.renderCertificates())));
    }
